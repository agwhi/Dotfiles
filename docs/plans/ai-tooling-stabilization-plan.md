...
| Claude Code | `/Users/alex/.local/bin/claude` -> `/Users/alex/.local/share/claude/versions/2.1.198` | `2.1.198 (Claude Code)` | manual/local now; target Homebrew cask `claude-code` | Yes, `cask "claude-code"` | Migration pending | Older local versions `2.1.187`, `2.1.196`, and `2.1.197` are also present under the local Claude versions tree. Migrate the live CLI to the stable Homebrew cask in a separate approved reinstall task. |
| Claude Desktop | `/Applications/Claude.app` | `1.18286.0` | manual app now; target Homebrew cask `claude` | Yes, `cask "claude"` | Migration pending | Replace or adopt through Homebrew in the same approved Claude harness reinstall task, without deleting sensitive `~/.claude` state. |
